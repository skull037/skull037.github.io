...
//var audioIcon = makeImage("Images/noAudio.png",700,300,50,50);
//var audioOn = false;
...
//document.getElementById("audio").addEventListener("click",start)
...
function removeArrayElementRect(array, index) {
  var element = array[index].rect
  if (element.parentNode) {
      element.parentNode.removeChild(element);
  }
  array.splice(index, 1)
}
function removeArrayElementBase(array, index) {
  var element = array[index].base
  if (element.parentNode) {
      element.parentNode.removeChild(element);
  }
  array.splice(index, 1)
...
function collide(player, obstacle, offsetX, offsetY) {
  ...
  if (!(getX(obstacle) + offsetX > getX(player) + Number(player.getAttribute("width"))/2
      ...
      || getY(obstacle) + Number(obstacle.getAttribute("height")) - offsetY < getY(player))) {
        return true
      }
  else {
    return false
...
function makeStorageImage(url, x, y, width, height, opacity) {
  var image = document.createElementNS(namespace, "image")
  image.setAttributeNS("http://www.w3.org/1999/xlink", "xlink:href", url)
  image.setAttribute("x", x)
  image.setAttribute("y", y)
...
gameOverRect.setAttribute( "opacity", size)
size+=0.0075;
  setTimeout(gameOverAnimation,10)
}
var size = 0;
